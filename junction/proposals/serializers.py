...
class ProposalFilterSerializer(serializers.Serializer):
    section = serializers.SlugRelatedField(slug_field='name', queryset=ProposalSection.objects.all(), required=False)
    type = serializers.SlugRelatedField(slug_field='name', queryset=ProposalType.objects.all(), required=False)
