...
class Explara(object):
    ...
    def get_events(self):
        events = requests.post(
            self.base_url.format("get-all-events"), headers=self.headers
        ).json()
        return [
            {"title": e.get("eventTitle"), "eventId": e.get("eventId")}
    ...
    def get_orders(self, explara_eventid):
        ...
        while not completed:
            ...
            if not attendee_response.get("attendee"):
                ...
                ticket_orders.extend(
                    [order for order in attendee_response.get("attendee")]
                )
            # after the first batch, subsequent batches are dicts with batch no. as key.
                ...
                ticket_orders.extend(
                    [
                        order
                        for order_idx, order in list(
                            attendee_response.get("attendee").items()
                        )
                    ]
                )
