...
class ConferenceFactory(Factory):
    ...
    start_date = fuzzy.FuzzyDate(
        datetime.date.today(), datetime.date(datetime.date.today().year + 1, 1, 1)
    ).fuzz()
...
def create_conference(**kwargs):
    ...
    if start_date and end_date:
        ...
        talks = ProposalTypeFactory.create(
            name="Talks", start_date=start_date, end_date=end_date
        )
...
def create_choice_feedback_question(**kwargs):
    ...
    ChoiceFeedbackQuestionValueFactory.create(question=question, title="Bad", value=0)
    ChoiceFeedbackQuestionValueFactory.create(question=question, title="Ok", value=1)
...
def create_feedback_questions(
    schedule_item_types, num_text_questions, num_choice_questions
):
    ...
    for _ in range(num_choice_questions):
        for item_type in item_types:
            obj = ChoiceFeedbackQuestionFactory.create(
                schedule_item_type=item_type, conference=conference, is_required=True
            )
            ChoiceFeedbackQuestionValueFactory.create(question=obj)
            ChoiceFeedbackQuestionValueFactory.create(question=obj)
            ChoiceFeedbackQuestionValueFactory.create(question=obj)
            choices.append(obj)
