...
def single_event():
	list_of_event = list()
	list_of_event.append({"eventId":"EKDJHH","eventTitle ":"My Demo Event",})
...
def single_order():
	...
	order["phoneNo"] = "22222"
	order["attendee"] = [{"ticketName":"Free Visit","ticketId":"TKEFAJC","name":"Pankaj Kumar","email":"dummy@mydummydomain.com","checkin":"no","ticketNo":"E4CACB-694","status":"attending","details":{"Date":""}}]
	list_of_order = list()
...
def single_order_no_ticketId():
	order = single_order()
	if "ticketId" in order[0]["attendee"] : del order[0]["attendee"]["ticketId"]
	return order

def single_order_no_attendee():
	order = single_order()
	if 'attendee' in order[0] : del order[0]['attendee']
	return order
